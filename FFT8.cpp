...
PoweredInteger divide(const BigInteger &first, const BigInteger &second, size_t precision) {
    ...
    if (temp != -1) {
        ...
        for (int iter = 1; static_cast<size_t>((1 << std::max(0, (iter - 3)))) < first.data.size() + precision;
             iter++) {
...
std::strong_ordering operator<=>(const Rational &first, const Rational &second) {
    ...
    if (first.p.sign == signs::neg && second.p.sign != signs::neg)
        return std::strong_ordering::less;
    if (first.p.sign == signs::zero)
        return 0 <=> static_cast<int>(second.p.sign);
...
long long primes[] = {100000007, 100000037, 100000039, 100000049, 100000073, 100000081, 100000123, 100000127,
...
long long fastmod(BigInteger x, long long p) {
    long long ans = 0;
    long long pwr = 1;
...
long long binpow(long long x, long long power, long long mod) {
    if (power == 0)
        return 1;
    long long k = binpow(x, power / 2, mod);
    k = (k * k) % mod;
    if (power % 2)
        k = (k * x) % mod;
    return k;
...
int main() {
    ...
    for (long long p : primes) {
        if (binpow(fastmod(x, p), (p - 1) / 2, p) != 1) {
            std::cout << "No";
            return 0;
        }
    }
